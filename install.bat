...
Write-Host @'
Author: @hugoxxdev
Website: https://hugoxx.sellix.io
'@`n
...
if (Test-Path -Path "temp\repo") {
  Get-ChildItem -Path "temp\repo" -Recurse | Remove-Item -Force -Recurse
  Remove-Item "temp\repo" -Force
}
if (Test-Path -Path "temp\repo-main") {
  Get-ChildItem -Path "temp\repo-main" -Recurse | Remove-Item -Force -Recurse
  Remove-Item "temp\repo-main" -Force
}

Write-Host 'Downloading files from GitHub repository'
Invoke-WebRequest -Uri 'https://github.com/hugoxx-official/repo/archive/main.zip' -OutFile 'temp.zip'
Expand-Archive 'temp.zip'
Remove-Item 'temp.zip'

Rename-Item -Path temp/repo-main -NewName SpotifyNoPremium
